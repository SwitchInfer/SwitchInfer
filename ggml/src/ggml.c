...
void * ggml_aligned_numa_malloc(size_t size, int node) {
    return numa_alloc_onnode(size, node);
}

void ggml_aligned_numa_free(void * ptr, size_t size) {
    numa_free(ptr, size);
}
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    ...
    *ctx = (struct ggml_context) {
        /*.mem_size           =*/ mem_size,
        /*.mem_buffer         =*/ params.mem_buffer ? params.mem_buffer : ggml_aligned_numa_malloc(mem_size, 0), // 这里调用 ggml_aligned_malloc 初始化 mem_buffer
    ...
    memset(ctx->mem_buffer, 0, mem_size);
...
void ggml_free(struct ggml_context * ctx) {
    ...
    if (ctx->mem_buffer_owned) {
        ggml_aligned_numa_free(ctx->mem_buffer, ctx->mem_size);
    }
