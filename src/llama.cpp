...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        // TODO: show n_nodes in gf
//        auto nodes = ggml_graph_nodes(gf);
//        int _n_nodes_ = ggml_graph_n_nodes(gf);
//        std::cout << _n_nodes_ << std::endl << std::endl;
        // TODO: show n_nodes in gf
        for (int il = 0; il < n_layer; ++il) {
          ...
          // TODO: show n_nodes in gf
//          int _nn_nodes_ = ggml_graph_n_nodes(gf);
//          std::cout << _nn_nodes_ << std::endl << std::endl;
//          for (int _i_ = _n_nodes_; _i_ < _nn_nodes_; _i_++) {
//            auto __node__ = nodes[_i_];
//            std::cout << __node__->name << ",  OP: " << __node__->op << std::endl;
//          }
//          std::cout << std::endl << std::endl;
//          _n_nodes_ = _nn_nodes_;
          // TODO: show n_nodes in gf
        }
...
static int llama_prepare_ubatch(
    ...
    if (hparams.causal_attn) {
        // FIXME: not to use llama_kv_cache_update, recover for normal inference
        ...
        if (!slot) {
            // TODO: normal execution goes here
            // return 1;
            return 0;
        }
...
static uint32_t n_tokens_processed = 0;
static bool is_first_decode = true;

static const uint32_t kv_head_reset  = 0;
static const uint32_t kv_n_reset     = KV_CACHE_LIMIT;
...
static int llama_decode_impl(
    ...
    while (lctx.sbatch.n_tokens > 0) {
        ...
        // reset kv cache if kv is too large
        const uint32_t prefill_token_threshold = 48;
        const uint32_t cur_token_num = lctx.sbatch.batch->n_tokens;
        assert(cur_token_num <= kv_n_reset);
        if (cur_token_num >= prefill_token_threshold) {
            kv_self.head = 0;
            kv_self.used = cur_token_num;
            kv_self.n    = cur_token_num + 16; // padding
        } else {
            if (n_tokens_processed > KV_CACHE_LIMIT) {
                n_tokens_processed = 0;
            }
            kv_self.head = n_tokens_processed;
            kv_self.used = n_tokens_processed + cur_token_num;
            kv_self.n    = n_tokens_processed + cur_token_num;
        }
        ...
        n_tokens_processed += cur_token_num;
...
//            if (kv_self.head >= KV_CACHE_LIMIT) {
//                kv_self.head = 0;
//            }
        }
...
// FIXME: normal cases, this function should not be called !!!
static void llama_kv_cache_update_impl(struct llama_context & lctx) {
    ...
    // FIXME: normal execution directly goes here
...
void llama_kv_cache_reset(struct llama_context * ctx) {
    ctx->kv_self.head = 0;
    ctx->kv_self.used = 0;
    ctx->kv_self.n    = 0;
}

bool llama_kv_cache_need_reset(struct llama_context * ctx) {
    const uint32_t kv_limit = 32;
    if (ctx->kv_self.head > kv_limit) {
        return true;
    }
    return false;
}
